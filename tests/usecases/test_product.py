...
async def test_usecases_get_should_return_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.get(id=UUID("0f8e7530-d0fe-4067-ae64-d2c74a45ddff"))

    assert (
        err.value.message
        == "Product not found with filter: 0f8e7530-d0fe-4067-ae64-d2c74a45ddff"
    )
